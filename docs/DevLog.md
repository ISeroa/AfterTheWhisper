...
- 매 `UpdateMoveTarget`에서 각 슬롯 비용을 계산해 최저 비용 슬롯 선택:
  ...
  Cost = Dist2D(SelfLoc, SlotPos)
       + OccupancyPenalty(400)  x (반경 OccupancyRadius(160) 안 실제 위치한 적 수)
       + SameSlotPenalty(100000) x (다른 ATDEnemyAIController 중 CurrentSlotIndex == i 인 수)
  ```
...
- `TryAttack(AActor* Target)` - `bOnCooldown` 또는 `DistSquared > AttackRange^2` 이면 즉시 return
